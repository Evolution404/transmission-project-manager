...
- 项目物资需求可以后补、增加、减少、换型并保留修订历史；已经发生的供应、出库、实施、有效结算事实不得被后续修订静默删除。
- 标准物资字典本身没有“已上报/已发货/已到货”状态；供应状态属于具体项目物资需求/供应批次，并且必须支持部分数量同时处于不同阶段。
- 至少保持 `到货数量 <= 发货数量 <= 上报数量`，并为需求降低到已发生供应事实以下提供显式调整/取消路径，不能直接覆盖历史。
- 凡提供批量导入的业务对象，必须同时提供手工新建入口；导入只是批量提效方式。
- 项目出库不等于物资仓库/供应状态。储备项目和后续执行项目共用业务 ID。
