...
- 当前开发阶段数据库只有 `0001_initial_schema.sql` 一个可重建基线；schema 变化直接修改 `0001` 并重建开发/测试数据库，禁止新增 `0002+` migration。历史生产数据的兼容属于发布期数据转换问题，不改变开发 schema 版本策略；只有用户明确宣布进入运行阶段/正式维护升级链后才冻结基线。
