...
P7-01 已明确以“系统标准模板下载 → 用户填报 → 回导”为主流程，不再等待任意历史 Excel 来定义字段；标准模板闭环可自动验证，最终只需真实业务值抽检。P7-02～13 仍缺相应真实业务资料、正式资源/授权、通知适配器和现场网络等证据。备份是多 tick JSON 分片，没有跨表快照隔离，迁移前需可靠停写和隔离恢复对账。当前没有年度/分类文件专用批量解析入口，按原件人工维护；`.xls` 仅作为兼容输入时需先转换。Excel 物理来源行号已修复空行/used-range 偏移，不自动重写旧批次。

当前 P7 准备门禁：100/100 Node/workerd+D1 + 58/58 Vue/Vitest，共 158/158 PASS；TypeScript、Vite build、Worker dry-run PASS。整条 `npm run check` 因工具单次 300 秒上限被截断，已按类型+构建+全部前端测试、Node 36+64 两组等价完整执行，无跳过。P7 已补标准需求模板下载/回导闭环、导入物理源行号修复，并新增统一 API 响应解析：开发代理/API 返回非 JSON 或畸形 JSON 时显示稳定中文错误，不暴露 `Unexpected token`；无迁移/认证模型变更。
