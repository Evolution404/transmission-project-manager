...
|---|---|---|---|---|
| P7-01 | 原始需求 Excel | 使用用户真实 .xlsx/UTF-8 .csv（.xls 需另存）验证工作表、字段映射、5 万行级分片、来源追溯、疑似重复和错误分类 | 阻塞：仓库无真实文件 | 抽检记录 + 导入结果 + 源行反查 |
| P7-02 | 一年工作早知道 | 按真实原件人工核对并录入年度事项，核对 month/day/unknown 日期精度及提醒日期 | 阻塞：真实资料未提供 | 事项抽检表 + 提醒结果 |
| P7-03 | 储备类别 | 按真实分类文件人工维护并核对需求类别→储备大类映射、共同费用分摊和金额守恒 | 阻塞：真实资料未提供 | 映射对照表 + 分类金额对账 |
...
当前仓库已经具备 P0–P6 的本地验证能力和 P7 离线准备，但尚未取得：

- 用户真实 `.xlsx/.xls/.csv`；
- `wrangler.production.jsonc` 或其他正式资源配置；
- 已授权启用的生产部署 GitHub workflow（当前仅有未启用模板及手工 preflight）；
- 经过真实复核的 P7 验收记录（现有 JSON/发布记录模板全部待填写）。

因此 P7 当前可以继续做生产前自检、验收记录模板和受保护部署流程的代码准备，但 P7-01～P7-13 中标记为“真实”的项目只有获得相应真实文件、云资源、域名、通知 provider 或现场网络后才能完成。不得用合成数据替代这些完成证据。

## 8. 本轮离线准备与源码审计结论

已完成的只是准备子项，不改变矩阵中 13 项真实验收状态：

- 非 Secret 生产 config 模板、拒绝占位/旧认证/额外覆盖的离线校验；
- 13 项验收证据 JSON 模板和结构检查，未完成退出 2，不能将合成结果填为真实 passed；
- 私有真实文件 SHA-256 清单工具、导入映射/原始行抽检操作表；
- P6 manifest/chunk/附件存在性离线检查，明确不等于一致性备份或恢复；
- 手工 production preflight（无云 Token、仅 dry-run）；生产部署模板默认不启用；普通 push 只跑测试；
- 发布、单独迁移、停写备份、隔离恢复、代码/数据回退、通知适配与运维撤权检查表；
- 修复 XLSX/CSV 表内空行及 XLSX used-range 起点导致的来源行号偏移，先失败回归后修复。

审计纠正：当前年度事项/储备类别通过页面维护，没有专用文件批量导入；旧 .xls 不直接支持；原始 Excel 文件不由现有导入 API 自动上传 R2；通知是通用 POST 适配器，不能将任意邮件厂商 URL 当作兼容接口。P6 备份为 JSON 分片，不是 SQL export，且没有跨表快照隔离；迁移前必须可靠停写、暂停并发任务、留存 schema/逐表基线并实际恢复对账。附件延迟删除/孤立对象周期清理和云用量告警尚未完成正式配置/实现，不能以业务预警代替。

操作和真实证据粒度见 [P7_RUNBOOK.md](P7_RUNBOOK.md)。工具通过不证明远端资源存在、审批可用、Token 归属、邮件送达、CPU 达标或真实数据正确。
