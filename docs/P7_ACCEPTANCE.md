...
|---|---|---|---|---|
| P7-01 | 标准需求模板回导 | 由系统下载标准 `.xlsx` 模板，用户按模板填报后回导；验证固定字段自动映射、5 万行级分片、来源追溯、疑似重复和错误分类。自定义字段映射只作为兼容能力，不以适配任意历史 Excel 为验收前提 | 部分完成：标准模板下载与自动回导合同已自动测试；真实业务值抽检待最终验收 | 模板版本/哈希 + 回导结果 + 源行反查 + 业务抽检记录 |
...
需求导入以系统发布的标准模板为主，不要求系统适配任意历史 Excel 的列名。真实填报数据如果暴露出新的业务字段、编码或类别，先判断是否应扩展标准模板/字典配置；只有确认现有业务模型确实不满足真实规则时，才按“测试先行 → 设计更新 → schema/API/UI 变更 → 完整门禁”的方式修改核心模型。
...
- 按系统标准模板填写的真实业务需求数据（只用于最终业务抽检，不用于反推字段结构）；
- `wrangler.production.jsonc` 或其他正式资源配置；
- 已授权启用的生产部署 GitHub workflow（当前仅有未启用模板及手工 preflight）；
- 经过真实复核的 P7 验收记录（现有 JSON/发布记录模板全部待填写）。

因此 P7 当前可以继续做生产前自检、验收记录模板和受保护部署流程的代码准备。P7-01 的结构/格式部分可由系统标准模板闭环自动验证，最终只需要用实际填报值抽样确认业务内容；其余标记为“真实”的项目仍需相应业务数据、云资源、域名、通知 provider 或现场网络后才能完成。不得用合成数据替代这些完成证据。
...
- 私有填报文件 SHA-256 清单工具、标准模板回导/原始行抽检操作表；
...
审计纠正：需求数据主流程为“系统下载标准模板 → 用户填报 → 浏览器解析并导入”，自定义字段映射仅保留兼容性；当前年度事项/储备类别通过页面维护，没有专用文件批量导入；旧 .xls 不直接支持；填报 Excel 文件不由现有导入 API 自动上传 R2；通知是通用 POST 适配器，不能将任意邮件厂商 URL 当作兼容接口。P6 备份为 JSON 分片，不是 SQL export，且没有跨表快照隔离；迁移前必须可靠停写、暂停并发任务、留存 schema/逐表基线并实际恢复对账。附件延迟删除/孤立对象周期清理和云用量告警尚未完成正式配置/实现，不能以业务预警代替。
