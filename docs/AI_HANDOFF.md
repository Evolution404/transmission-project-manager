...
P0、P1、P1.1、P1.2、P2、P3 已完成并通过本地验收。业务登录现为系统自维护 `username + password`：浏览器 Web Worker 执行 Argon2id，服务端保存带运行时 pepper 的 HMAC verifier，并使用 7 天 HttpOnly 服务端会话。邮箱和 Cloudflare Access 均不参与业务认证。
...
**下一步进入 P4：框架、协议、预算与费用流水。** Cloudflare 仍是托管路线，但不参与业务身份认证；继续保留 P1.2 的账号、会话和权限中间件，不得把旧 Access/邮箱身份代码带回来。对外简短交接直接使用 `docs/NEXT_AI.md`。
...
- P3 完整本地门禁已全绿：58/58 Node/workerd+D1 + 32/32 Vue/Vitest，共 90 项；覆盖 P1.2/P2 全量回归、P2→P3 数据保留、数量守恒与并发超分配、估算定点计算、缺价/零价、分类金额守恒、储备版本历史、105+ 候选游标分页及 `/reserves` 页面合同。CI 实际结果以仓库 Actions 为准。
