...
尚未实现：P1.1 成员新增/编辑/范围授权、首次登录状态、首个管理员 bootstrap、最后管理员保护和可移交运维闭环；以及 Excel导入、需求/储备/物资业务表、框架协议预算、出库实施结算、分类分析、定时任务、发信、备份及正式Cloudflare部署。后续业务入口当前均为真实空状态，不得把页面框架解释为这些业务已完成。

**下一步从 P1.1 开始，完成后再进入 P2。** 不要重新搭建仓库或推翻已确认的 Cloudflare 路线。
...
P0、P1 核心已完成。按实施计划先完成 P1.1（成员管理、首次登录、首管理员保护、可移交运维），
不要重做 P1 身份框架；P1.1 完成后再进入 P2，也不要将后续未实现能力标为完成。
保持 Cloudflare 免费起步、Vue + TypeScript + Hono + D1 + R2 的架构。
不需要再次询问已经在文档中确定的需求。原始Excel缺失时使用明确标注的合成测试数据，
P1.1 必须保证日常业务成员管理不依赖资产所有者 Cloudflare 登录；Cloudflare 技术维护使用受托人的独立成员身份，
正式 CI/CD 的 account-owned token 和完整移交演练留到 P7。P1.1 完成后运行 npm run check 和该阶段验收，更新实施计划与交接说明，
...
- 下一步：P1.1 成员管理与可移交运维；完成后进入 P2 Excel导入、需求池与物资字典。
