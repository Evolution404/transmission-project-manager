...
P0、P1、P1.1、P1.2、P2、P3、P4 已完成并通过本地验收。业务登录现为系统自维护 `username + password`：浏览器 Web Worker 执行 Argon2id，服务端保存带运行时 pepper 的 HMAC verifier，并使用 7 天 HttpOnly 服务端会话。邮箱和 Cloudflare Access 均不参与业务认证。
...
**下一步进入 P5：出库、实施、结算与四状态反馈。** Cloudflare 仍是托管路线，但不参与业务身份认证；继续保留 P1.2 的账号、会话和权限中间件，不得把旧 Access/邮箱身份代码带回来。对外简短交接直接使用 `docs/NEXT_AI.md`。
...
- 完成：框架/协议当前态及历史版本、项目框架归属、预算草稿/确认版本、多协议预算分配、预算发生/实际发生独立流水、冲销与资金汇总；`/finance` 为真实懒加载页面。
