...
**下一步不再直接进入 P7。** 用户在 2026-09-12 明确纠正了需求/项目/物资核心业务模型：需求/隐患台账可以包含当时已知的物资信息，但不等于最终项目物资清单；项目储备允许 0 条物资，项目物资可持续修订，并需要按数量追踪“已上报/已发货/已到货”等供应状态。参考 WorkBuddy 页面已通过本机 Chrome CDP 实际读取并校准，详细结论见 `docs/HANDOFF-PROJECT-MODEL-REFACTOR-2026-09-12.md`。先完成本次模型重构，再继续 P7。Cloudflare 仍是托管路线，但不参与业务身份认证；P1.2 认证基线不得回退。对外简短交接直接使用 `docs/NEXT_AI.md`。
...
- 旧 `demand_allocations` 数量守恒模型不再作为最终业务基线。需求与项目的来源关联、项目当前物资需求、物资供应状态必须拆开建模；详细规则以 `HANDOFF-PROJECT-MODEL-REFACTOR-2026-09-12.md` 为准。
