...
**下一步不再直接进入 P7。** 用户在 2026-09-12 明确纠正了需求/项目/物资核心业务模型：需求/隐患台账是纯抽象业务事项，本体不依赖物资，但可以附带 0..N 条当时已知的物资明细；这些物资不等于最终项目物资清单。项目储备允许 0 条物资，项目物资可持续修订；**项目出库是项目级进入执行阶段的节点，出库后一个项目可以有多个执行任务，每个任务都有自己的物资、物资供应、实施和结算；供应/实施/结算三条线并行推进。** 物资供应按任务物资数量追踪“已上报/已发货/已到货”等状态。参考 WorkBuddy 页面已通过本机 Chrome CDP 实际读取并校准，详细结论见 `docs/HANDOFF-PROJECT-MODEL-REFACTOR-2026-09-12.md`。先完成本次模型重构，再继续 P7。Cloudflare 仍是托管路线，但不参与业务身份认证；P1.2 认证基线不得回退。对外简短交接直接使用 `docs/NEXT_AI.md`。
...
- 旧 `demand_allocations` 数量守恒模型和 P5 的“项目物资 release line 直接串实施/结算”都不再作为最终业务基线。需求关联、项目总体物资、项目级出库事件、执行任务、任务物资、任务供应/实施/结算必须拆开建模；详细规则以 `HANDOFF-PROJECT-MODEL-REFACTOR-2026-09-12.md` 为准。
