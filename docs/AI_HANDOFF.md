...
无头浏览器验收基础设施已经补齐并进入 CI：仓库已引入 Playwright，`npm run test:ui:headless` 每次使用临时目录创建隔离 D1 / R2 本地状态并启动独立 Wrangler Worker，不复用开发数据库、不读取真实浏览器 Cookie，也不接触真实账号。认证链路完全通过页面完成：先在初始化页输入测试管理员姓名、测试账号、测试密码和测试初始化令牌创建首管理员；随后开启新的无 Cookie 浏览器会话，再从登录页输入同一测试账号密码完成真实登录，覆盖浏览器 Argon2id 派生、服务端 HMAC verifier 与 HttpOnly session。后续视觉验收复用的是该真实登录产生的 HttpOnly 会话状态，而不是伪造 Cookie。当前 Headless Chromium **20/20 PASS**：除初始化、重新登录和深链刷新外，正式覆盖 1440 桌面明暗主题、1440×600 低高度、768 / 900 / 1100 紧凑桌面、768×600 低高度、390 手机明暗主题和 320px 最窄手机；所有 8 个主路由必须真实加载成功，出现可见加载错误态即失败。验收持续检查横向溢出、实际字号下限、手机触控目标、系统明暗 token、侧栏/账号/手机导航元素重叠，并校验“新增需求 / 新建项目 / 新增成员”在 1440、768、390、320 四档视口内完整可见。E2E 还会创建隔离项目与任务，通过真实业务页面打开项目物资、项目来源、供应、实施、结算 5 类 Drawer，分别在 390px 和 320px 触控视口中验证完整进入屏幕。GitHub Actions `CI` 保留独立 `headless-ui` job，失败时上传 screenshot/trace。

本轮浏览器级验收继续发现并修复了静态审计难以发现的真实问题：手机顶部重复退出入口只有 32px 命中高度；任务页 text-only“重新加载”实际仅 14px，高频错误恢复动作已统一改为真实按钮并加静态门禁；设置页“新增大类 / 设置映射”手机按钮从 32px 提升到可触控高度；768×700 紧凑桌面下“新增需求”曾高达 **1207.7px**，现将 Teleport 模态框最大高度/内部滚动提升为通用规则而非只在 ≤767px 生效；线路删除不再只依赖 hover 快捷操作，详情“更多操作”提供稳定入口。进一步审计还发现项目物资、项目来源、供应、实施、结算 Drawer 都把 class 直接挂在 `NDrawer` 根节点，却错误使用 `.xxx-drawer .n-drawer` 后代选择器，现已统一改为 `.xxx-drawer.n-drawer` 并新增 repository guard。E2E 新增“主路由不得处于加载错误态”后还暴露出任务队列真实后端缺陷：SQL 仍读取已不存在的 `projects.year`，已独立修复为当前 schema 的 `business_year`，repository fixture 同步纠正。最新完整 `npm run check` 为 Node **317/317 PASS**、Web **165/165 PASS（23 文件）**，TypeScript、Web production build、Worker dry-run 与 Node + SQLite + Filesystem 第二运行时全部通过；独立 Headless Chromium 为 **20/20 PASS**。当前分支最新 UI 收口仍需以本次提交后的 GitHub CI 结果作为最终远端证据。
