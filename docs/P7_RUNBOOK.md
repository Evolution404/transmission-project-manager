...
此版本只完成离线准备；没有创建正式资源、配置 Secret、发信或部署。正式操作必须获得当次授权。P7-01～13 的完成状态仍由真实证据决定。
...
npm run p7 -- inputs data/private/需求.xlsx data/private/需求.csv
...
- `inputs` 流式计算文件名、大小、SHA-256，旧 `.xls` 标记需转换并退出 2；不输出行内容，不上传、不导入、不证明映射正确。原件及转换件都保存，抽检使用应用现有解析/映射流程，不另造第二套解析规则。
...
资料由业务负责人选择代表性项目，使用只读原件；转换或修订文件必须保留原件哈希和变更说明。每个样本记录文件哈希、工作表、物理行、批次 ID、需求 ID、项目 ID、预期值、实际值、复核人及结论，记录留私有目录。

| 验收编号 | 必须核对的边界 |
|---|---|
| 01 | 六必填字段、单位/年度/类别扩展；多工作表、表内空行、非第一行起始区域；实际源行反查；未知型号/电压、负数和过精度数量；同源恢复/疑似重复；20 行上传/校验和 10 行发布；真实 5 万行/5 人并发单列记录，不能用小样本替代 |
| 02 | 原文件事项先人工核对再用事项页录入；目前没有年度文件批量解析入口。month/day/unknown、负责人、提前天数、跨月/跨年、完成停止提醒；仅月份不补具体日 |
| 03 | 原分类表人工核对并维护类别/映射；目前没有分类文件批量解析入口。同型号不同单位、共同费用分摊、缺价/null 与明确零价、部分出库剩余金额、未分类和金额守恒 |
| 04 | 框架/协议/预算版本、有效期与归属；预算确认不产生发生；多协议分摊、冲销；90%/80%及超 1 分边界；以整数分对账且各账目独立 |
| 05 | 出库/实施/结算实际凭证；部分完成、先结算后实施、最终结算/撤销；历史未关联不补造出库；四状态、范围保护、附件及跨范围拒绝 |
...
6. 对所有业务表逐表计数；项目/需求数量守恒、分类估算、按框架/协议/项目/类型的整数分流水、出库/实施/结算范围和四状态、规则版本及历史报告逐项对账；运行 `PRAGMA foreign_key_check`，确认会话为 0，附件每个可授权下载且跨项目拒绝。
