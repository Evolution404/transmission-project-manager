...
当前进度（2026-09-14）：**M1–M4 已完成，进入 M5。** M1 已完成编号规范化与单基线 schema；M2 已完成专用更名、历史追踪和歧义搜索；M3 已完成按杆塔编号自动初始插位、专用前/后移动、稀疏排序与 `tower_order_version`；M4 已完成 Excel/CSV/粘贴全量预检、用户侧不限行、内部幂等分片/断点继续，以及“完整清单才允许按文件顺序原子重排”。下一步只剩 M5：彻底移除三栏 UI，改为线路中心首页与全宽线路详情，并把更名、导入、调整顺序入口完整呈现。
