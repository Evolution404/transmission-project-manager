...
当前进度（2026-09-14）：**M1、M2 已完成，M3 开始。** M1 已将生产代码和测试从 `sort_index` 完整迁到 `sort_rank`，加入统一杆塔编号规范化、历史表 schema、`tower_order_version`，并取消线路名/杆塔号唯一约束。M2 已增加专用线路/杆塔更名原子接口、历史查询和当前/历史搜索；普通 PATCH 无法绕过历史链；同名/同编号导入会明确报歧义而不是随机选择。M3 将把排序从普通 PATCH/批量技术字段中进一步抽离，建立用户语义的“移动到前/后”和稀疏排序版本控制。
