...
无头浏览器验收已改为仓库内可重复执行的 Playwright E2E，而不再依赖用户真实 Chrome。`npm run test:ui:headless` 会先构建 Web，然后在临时目录初始化独立本地 D1/R2 状态并启动独立 Wrangler Worker；测试凭据只存在于 E2E 代码与临时进程中，不读取 `.env` 中真实账号密码、不复制用户真实 Cookie。认证必须走页面本身：首次初始化输入测试账号/密码/初始化令牌，新浏览器会话再次输入账号密码登录；后续视口/主题验收只复用这次真实登录产生的会话状态。当前 Headless Chromium **10/10 PASS**，覆盖业务深链整页刷新后的会话/路由保持、desktop-light、desktop-dark、**900px compact-light**、mobile-light、mobile-dark，并在登录后遍历工作台、需求、项目、任务、资金、分析、基础台账、设置 8 个主路由。验收不仅检查横向溢出、实际计算字号最低 12px、手机主导航触控高度最低 44px、明暗主题 token 和关键导航交互，还使用真实 DOMRect 拦截侧栏激活竖条/图标、侧栏图标/文字、右上账号区、手机底部导航、手机“更多”菜单及手机账号行的元素重叠；桌面/手机继续校验“新增需求 / 新建项目 / 新增成员”弹层完整位于视口内。浏览器验收已实际发现并修复：设置页 DataTable 动态单元格 scoped CSS 未命中导致账号字号仅 10.83px；手机“新建项目”抽屉保留 520px 桌面宽度；需求模态框 Teleport 后 scoped `:deep()` 高度规则未命中，390×844 视口中“新增需求”曾达到 1267.16px；侧栏激活竖条被滚动容器裁剪以及 `AppPressable` 高优先级 reset 导致竖条与图标实际重叠 6px；900px 紧凑桌面设置页成员表还曾把整页撑到 980px。当前 `AppPressable` reset 使用低优先级 `:where()`，侧栏保留独立 indicator gutter，设置页 grid 子项可收缩且成员表仅在自身内部横向滚动；Shell 身份统一为 `username` 主信息、角色次信息，桌面右上不再重复角色。CI 已增加独立 `headless-ui` job，并由 repository guard 锁定 Chromium 安装和 E2E 执行入口；失败时自动上传 Playwright screenshot/trace 诊断 artifact。同期完整 `npm run check` 为 Node **314/314 PASS**、Web **165/165 PASS（23 文件）**。后续 UI 改动必须同时维持该 E2E 与现有单元/集成门禁全绿。
