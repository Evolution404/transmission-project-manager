...
当前进度（2026-09-14）：**M1 已完成，M2 开始。** M1 已将生产代码和测试从 `sort_index` 完整迁到 `sort_rank`，加入统一杆塔编号规范化、历史表 schema、`tower_order_version`，并取消线路名/杆塔号唯一约束；需求导入可接受 `#1` 等简写并解析为标准编号。M2 将专门收紧普通 PATCH，增加线路/杆塔更名原子接口和历史名称/编号查询。
