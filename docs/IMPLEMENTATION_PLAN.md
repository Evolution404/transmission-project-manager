...
无头浏览器验收已改为仓库内可重复执行的 Playwright E2E，而不再依赖用户真实 Chrome。`npm run test:ui:headless` 会先构建 Web，然后在临时目录初始化独立本地 D1/R2 状态并启动独立 Wrangler Worker；测试凭据只存在于 E2E 代码与临时进程中，不读取 `.env` 中真实账号密码、不复制用户真实 Cookie。认证必须走页面本身：首次初始化输入测试账号/密码/初始化令牌，新浏览器会话再次输入账号密码登录；后续视口/主题验收只复用这次真实登录产生的会话状态。当前 Headless Chromium **9/9 PASS**，覆盖业务深链整页刷新后的会话/路由保持、desktop-light、desktop-dark、mobile-light、mobile-dark，并在登录后遍历工作台、需求、项目、任务、资金、分析、基础台账、设置 8 个主路由，检查横向溢出、实际计算字号最低 12px、手机主导航触控高度最低 44px、明暗主题 token 和关键导航交互；桌面/手机还分别校验“新增需求 / 新建项目 / 新增成员”弹层完整位于视口内。浏览器验收实际发现并修复了设置页 DataTable 动态单元格 scoped CSS 未命中导致账号字号仅 10.83px，以及手机“新建项目”抽屉错误保留 520px 桌面宽度两个问题。CI 已增加独立 `headless-ui` job，并由 repository guard 锁定 Chromium 安装和 E2E 执行入口。同期完整 `npm run check` 为 Node **312/312 PASS**、Web **165/165 PASS（23 文件）**；后续 UI 改动必须同时维持该 E2E 与现有单元/集成门禁全绿。
