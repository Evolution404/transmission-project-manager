...
- 业务操作不得使用 Naive UI `tiny` 按钮；文本式返回/返回上级导航仍可保持轻量外观，但桌面命中高度至少 36px，手机至少 44px，不能让字号提升后点击区域仍只剩一行文字。
...
- 项目来源需求、项目物资等带版本守卫的编辑器遇到 409 时必须保留用户草稿，并提供原地读取最新项目数据的明确动作；所有主要写操作弹窗/抽屉在 mutation 进行中都必须拒绝遮罩、右上角关闭、Esc / `update:show` 与取消操作，禁止中断界面状态。基础台账等拥有大量同类弹窗的页面应共享关闭门禁状态，不复制多套逻辑。
- 资金、分析、需求导入/物资字典等内联编辑工作区在 mutation 进行中必须锁定决定请求语义的业务上下文和当前表单字段；不得允许切框架、切项目、改映射、切主 Tab 或折叠当前表单后再让旧请求结果回写到新上下文。所有行级状态写入也必须有明确进行中反馈并阻止重复提交。
- 资金/分析等可快速切换框架、项目、统计日期的工作区必须测试异步响应乱序：人为让旧请求最后返回，页面仍只能保留最新框架/项目/日期的数据；资金流水“加载更多”也不得把旧框架游标结果追加到新框架。
- 最新框架/项目/统计日期请求失败必须进入页面内错误恢复，不得形成未处理 Promise；“重新加载”必须重试当前选择而不是回到初始化默认值。旧上下文请求即使失败也不得在新上下文弹错误。分析等可视化页面切换上下文后若读取失败，旧上下文图表必须清空，禁止“错误提示 + 旧业务图”同时存在。
- 对“mutation 成功 → 随后 reload 失败”必须单独测试：写入成功后不得走 mutation 的 `message.error`；应提示“已保存/已创建/已确认，但最新数据刷新失败，请重新加载”，并让页面错误恢复入口承担后续同步。金额、预算、规则等高风险写入优先覆盖，避免用户因误判失败而重复提交已生效业务事实。
- 需求域同样执行上述规则：手工需求创建、需求物资追加、标准物资新增、批量导入发布均不得因后续列表/字典刷新失败而误报 mutation 失败。至少保留一条真实行为测试模拟“POST 已成功、随后 GET 失败”，并用 repository guard 锁定全部需求写路径。
